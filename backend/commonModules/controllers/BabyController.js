...
const getBabyById = async (req, res) => {
  try {
    ...
    const baby = await Baby.findOne({ _id: req.params.id, user: req.user._id, isActive: true })
      .populate("babyStage", "title features")
      .populate("selectedCountries", "name signatureFoods");

    if (!baby) {
      return sendResponse({ res, statusCode: 404, translationKey: "baby_not_found" });
    }
  ...
  try {
    ...
    if (stageId !== undefined) {
      if (stageId) {
        const stage = await BabyStage.findOne({ _id: stageId, active: true });
        if (!stage) {
          return sendResponse({ res, statusCode: 404, translationKey: "baby_stage_not_found" });
        }
      }
    ...
    if (countryIds !== undefined) {
      if (countryIds.length > 2) {
        return sendResponse({ res, statusCode: 400, translationKey: "max_two_countries_allowed" });
      }
      if (countryIds.length > 0) {
        const countries = await Country.find({ _id: { $in: countryIds }, isEnabled: true });
        if (countries.length !== countryIds.length) {
...
const deleteBaby = async (req, res) => {
  try {
    ...
    if (String(user.activeBaby) === String(baby._id)) {
      const nextBaby = await Baby.findOne({ user: req.user._id, isActive: true });
...
const switchActiveBaby = async (req, res) => {
  try {
    if (!validateParams(req, res, { rawData: ["babyId"], objectIdFields: ["babyId"] })) return;

    const { babyId } = req.body;

    const baby = await Baby.findOne({ _id: babyId, user: req.user._id, isActive: true });
    if (!baby) {
      return sendResponse({ res, statusCode: 404, translationKey: "baby_not_found" });
    }
